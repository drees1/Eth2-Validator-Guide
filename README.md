...
Note: You can generate keys for multiple validators at once, your use `existing-mnemonic` command to generate more validator keys later under the same mnemonic.
...
- Afterwards, they will show `Deposit processed, entering activation queue after finalization` until the validator is activated
...
Connect to server via SSH with port forwarding for prysm webUI and logs:
```sh
...
Access the webUI from a browser on remote computer:
```
http://localhost:7500
```
